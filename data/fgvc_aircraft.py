...
def make_dataset(dir, image_ids, targets):
    ...
    for i in range(len(image_ids)):
        item = (i, os.path.join(dir, 'data', 'images',
...
class FGVCAircraft(Dataset):
    ...
    def __init__(self, root, class_type='variant', split='train', transform=None,
        ...
        (image_ids, targets, classes, class_to_idx) = find_classes(self.classes_file)  # 6667, 6667, 100, 100
...
def subsample_dataset(dataset, idxs):
    ...
    dataset.data = [(index, p, t) for i, (index, p, t) in enumerate(dataset.data) if i in idxs]
...
def subsample_classes(dataset, include_classes=range(60)):
    ...
    cls_idxs = [i for i, (index, p, t) in enumerate(dataset.data) if t in include_classes]
...
def get_train_val_indices(train_dataset, val_split=0.2):
    ...
    all_targets = [t for i, (index, p, t) in enumerate(train_dataset.data)]
...
def get_aircraft_datasets(train_transform, test_transform, train_classes=range(50), prop_train_labels=0.8,
    ...
    whole_training_set = FGVCAircraft(root=aircraft_root, transform=train_transform, split='trainval')  # len = 6667
...
def main():
    ...
    print(len(set([i[1] for i in x['train_labelled'].data])))
    print('Printing total number of classes...')
    print(len(set([i[1] for i in x['train_unlabelled'].data])))
